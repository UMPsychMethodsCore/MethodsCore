...
% Karl Friston
% $Id: spm_fMRI_design_show.m 2654 2009-01-26 02:32:57Z Darren $
...
h =patch([0 1 1 0]/HPF,[0 0 1 1]*max(max(gX)),[1 1 1]*.9);

% Set the buttondownfcn of the patch to change the figure renderer to
% zbuffer. This allows one one to see the overlap between the HPF patch and
% the spectral frequency graph. The renderer is reset after 2 seconds. This
% avoids errors in some OS's that occur with non-painter renderers. DRG
%------------------------------------------------------------------------
set(h,'ButtonDownFcn',['Fgraph=spm_figure(''GetWin'',''Graphics'');',...
    'rend=get(Fgraph,''Renderer'');',...
    'set(Fgraph,''Renderer'',''zbuffer'');',...
    'drawnow;',...
    'pause(2);',...
    'set(Fgraph,''Renderer'',rend)']);
