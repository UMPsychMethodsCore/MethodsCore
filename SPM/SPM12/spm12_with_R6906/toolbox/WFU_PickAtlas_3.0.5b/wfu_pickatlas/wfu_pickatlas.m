...
% Last Modified by GUIDE v2.5 16-May-2014 11:19:58
...
wfu_require_spm_lite({'SPM8','SPM12'});
%warning off;

if nargin <=1  % LAUNCH GUI
  WFU_LOG.info('Entered function $Revision: 3.7 $');
...
%    LookUpFileName=fullfile(handles.data.atlas_toolbox, handles.data.SelectedAtlasType.subdir, handles.data.SelectedAtlasType.lookupfile);
    handles.AtlasMenuString= wfu_implode({Atlas(1:end-1).Name},'|');
...
%		global ben;
    ...
    %Setup handles for MAG button
    handles.DispImg=DispImg;
    handles.DispImgSag=rot90(squeeze(handles.data.DispVolWork(origin(1),:,:)));
    handles.DispImgCor=rot90(squeeze(handles.data.DispVolWork(:,origin(2),:)));
    
    
		handles.data.figure1=handles.WFU_PickAtlas;
    handles.magFigure=[];
    handles.magFigureAxes={[] [] []};
...
%	control components need to call this function to update display)
    ...
    %cal size of "green dot"
    xMore=floor((handles.data.iheader.dim(1)-100)/100);
    if xMore < 0, xMore=0; end
    yMore=floor((handles.data.iheader.dim(2)-100)/100);
    if yMore < 0, yMore=0; end
    zMore=floor((handles.data.iheader.dim(3)-100)/100);
    if zMore < 0, zMore=0; end
    ...
    posz=handles.selectedpoint.z;
    
    xGrid=posx-xMore:posx+xMore;
    xGrid=xGrid(xGrid>0 & xGrid <= handles.data.iheader.dim(1));

    yGrid=posy-yMore:posy+yMore;
    yGrid=yGrid(yGrid>0 & yGrid <= handles.data.iheader.dim(2));

    zGrid=posz-zMore:posz+zMore;
    zGrid=zGrid(zGrid>0 & zGrid <= handles.data.iheader.dim(3));

    
%    if(handles.selectedpoint.z == handles.slice)
%      handles.DispImg(yGrid,xGrid) = handles.data.greencolor;
%        handles.DispImg(posy,posx) = handles.data.greencolor;
%    end

    handles.DispImg(yGrid,xGrid) = handles.data.greencolor;
    
    handles.DispImgSag=squeeze(handles.data.DispVolWork(posx,:,:));
    handles.DispImgSag(yGrid,zGrid) = handles.data.greencolor;
    handles.DispImgSag=rot90(handles.DispImgSag);
    
    handles.DispImgCor=squeeze(handles.data.DispVolWork(:,posy,:));
    handles.DispImgCor(xGrid,zGrid) = handles.data.greencolor;
    handles.DispImgCor=rot90(handles.DispImgCor);
    
    set(handles.img, 'CData', handles.DispImg);
    if all(~cellfun(@isempty,handles.magFigureAxes)) && all(~cellfun(@isempty,cellfun(@ishandle,handles.magFigureAxes,'UniformOutput',false)))
%      data=flipud(get(handles.img,'CData'));
%      set(handles.magFigureAxes,'CData',data);

      map=colormap(get(handles.img,'Parent'));
      imshow(handles.DispImgCor,map,'Parent',handles.magFigureAxes{1});
      imshow(handles.DispImgSag,map,'Parent',handles.magFigureAxes{2});
      imshow(flipud(handles.DispImg),map,'Parent',handles.magFigureAxes{3});
      set(handles.magFigureSliders{1},'Value',str2double(get(handles.txtCubeX,'String')));
      set(handles.magFigureSliders{2},'Value',str2double(get(handles.txtCubeY,'String')));
      set(handles.magFigureSliders{3},'Value',handles.slice);
      set(handles.magFigureEdit{1},'String',get(handles.txtCubeX,'String'));
      set(handles.magFigureEdit{2},'String',get(handles.txtCubeY,'String'));
      set(handles.magFigureEdit{3},'String',num2str(handles.slice));
    end
    guidata(handles.WFU_PickAtlas,handles);
...
function magButton_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton24 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global WFU_LOG;
if isempty(handles.magFigure) || ~ishandle(handles.magFigure)
  handles.magFigure=figure;
  figure(handles.magFigure);

%  map=colormap(get(handles.img,'Parent'));
%  data=flipud(get(handles.img,'CData'));
%  handles.magFigureAxes=imshow(data,map);

  handles.magFigureAxes={subplot_tight(2,2,1) subplot_tight(2,2,2) subplot_tight(2,2,3)};
  map=colormap(get(handles.img,'Parent'));
  ...
  handles.magFigureLabels{1} =uicontrol(handles.magFigure,'Style','Text',  'Units','Normalized','Position',[.55 .40 .05 .05],'String','X');
  handles.magFigureLabels{2} =uicontrol(handles.magFigure,'Style','Text',  'Units','Normalized','Position',[.55 .30 .05 .05],'String','Y');
  handles.magFigureLabels{3} =uicontrol(handles.magFigure,'Style','Text',  'Units','Normalized','Position',[.55 .20 .05 .05],'String','Z');
  handles.magFigureSliders{1}=uicontrol(handles.magFigure,'Style','Slider','Units','Normalized','Position',[.60 .40 .30 .05],'Min',1,'Max',handles.data.iheader.dim(1),'Value',str2double(get(handles.txtCubeX,'String')));
  handles.magFigureSliders{2}=uicontrol(handles.magFigure,'Style','Slider','Units','Normalized','Position',[.60 .30 .30 .05],'Min',1,'Max',handles.data.iheader.dim(2),'Value',str2double(get(handles.txtCubeY,'String')));
  handles.magFigureSliders{3}=uicontrol(handles.magFigure,'Style','Slider','Units','Normalized','Position',[.60 .20 .30 .05],'Min',1,'Max',handles.data.iheader.dim(3),'Value',handles.slice);
  handles.magFigureEdit{1}   =uicontrol(handles.magFigure,'Style','Edit',  'Units','Normalized','Position',[.90 .40 .05 .05],'String',get(handles.txtCubeX,'String'));
  handles.magFigureEdit{2}   =uicontrol(handles.magFigure,'Style','Edit',  'Units','Normalized','Position',[.90 .30 .05 .05],'String',get(handles.txtCubeY,'String'));
  handles.magFigureEdit{3}   =uicontrol(handles.magFigure,'Style','Edit',  'Units','Normalized','Position',[.90 .20 .05 .05],'String',num2str(handles.slice));
  handles.magFigureStatus    =uicontrol(handles.magFigure,'Style','Text',  'Units','Normalized','Position',[.55 .10 .40 .05],'String','');
  
  set(handles.magFigureSliders{1},'Callback',@(hObject,eventdata)wfu_pickatlas('MagSliderX_Callback',hObject,eventdata,guidata(hObject)));
  set(handles.magFigureSliders{2},'Callback',@(hObject,eventdata)wfu_pickatlas('MagSliderY_Callback',hObject,eventdata,guidata(hObject)));
  set(handles.magFigureSliders{3},'Callback',@(hObject,eventdata)wfu_pickatlas('MagSliderZ_Callback',hObject,eventdata,guidata(hObject)));
  set(handles.magFigureEdit{1},   'Callback',@(hObject,eventdata)wfu_pickatlas('MagEditX_Callback',hObject,eventdata,guidata(hObject)));
  set(handles.magFigureEdit{2},   'Callback',@(hObject,eventdata)wfu_pickatlas('MagEditY_Callback',hObject,eventdata,guidata(hObject)));
  set(handles.magFigureEdit{3},   'Callback',@(hObject,eventdata)wfu_pickatlas('MagEditZ_Callback',hObject,eventdata,guidata(hObject)));
  set(handles.magFigure,          'WindowButtonDownFcn',@(hObject,eventdata)wfu_pickatlas('MagMouseDown_Callback',hObject,eventdata,guidata(hObject)));
  set(handles.magFigure,          'WindowButtonMotionFcn',@(hObject,eventdata)wfu_pickatlas('MagMouseMvmt_Callback',hObject,eventdata,guidata(hObject)));
  
end
guidata(handles.WFU_PickAtlas, handles);
guidata(handles.magFigure,struct('WFU_PickAtlas',handles.WFU_PickAtlas));


function MagSliderX_Callback(hObject, eventdata, handles)
  handles=guidata(handles.WFU_PickAtlas); %get the actual handles from the main window
  set(handles.txtCubeX, 'string', num2str(round(get(handles.magFigureSliders{1},'Value'))));
  set(handles.magFigureEdit{1},'String',get(handles.txtCubeX,'String'));
  cmdGo1_Callback([], [], handles, [])

function MagSliderY_Callback(hObject, eventdata, handles)
  handles=guidata(handles.WFU_PickAtlas); %get the actual handles from the main window
  set(handles.txtCubeY, 'string', num2str(round(get(handles.magFigureSliders{2},'Value'))));
  set(handles.magFigureEdit{2},'String',get(handles.txtCubeY,'String'));
  cmdGo1_Callback([], [], handles, [])

function MagSliderZ_Callback(hObject, eventdata, handles)
  handles=guidata(handles.WFU_PickAtlas); %get the actual handles from the main window
  set(handles.txtCubeZ, 'string', num2str(round(get(handles.magFigureSliders{3},'Value'))));
  set(handles.magFigureEdit{3},'String',get(handles.txtCubeZ,'String'));
  cmdGo1_Callback([], [], handles, [])

function MagEditX_Callback(hObject, eventdata, handles)
  handles=guidata(handles.WFU_PickAtlas); %get the actual handles from the main window
  v=round(str2double(get(handles.magFigureEdit{1},'String')));
  set(handles.txtCubeX, 'string', num2str(v));
  set(handles.magFigureEdit{1},'String',get(handles.txtCubeX,'String'));
  set(handles.magFigureSliders{1},'Value',v);
  cmdGo1_Callback([], [], handles, [])

function MagEditY_Callback(hObject, eventdata, handles)
  handles=guidata(handles.WFU_PickAtlas); %get the actual handles from the main window
  v=round(str2double(get(handles.magFigureEdit{2},'String')));
  set(handles.txtCubeY, 'string', num2str(v));
  set(handles.magFigureEdit{2},'String',get(handles.txtCubeY,'String'));
  set(handles.magFigureSliders{2},'Value',v);
  cmdGo1_Callback([], [], handles, [])

function MagEditZ_Callback(hObject, eventdata, handles)
  handles=guidata(handles.WFU_PickAtlas); %get the actual handles from the main window
  v=round(str2double(get(handles.magFigureEdit{3},'String')));
  set(handles.txtCubeZ, 'string', num2str(v));
  set(handles.magFigureEdit{3},'String',get(handles.txtCubeZ,'String'));
  set(handles.magFigureSliders{3},'Value',v);
  cmdGo1_Callback([], [], handles, [])

function [X Y Z] = GetMagMouseLoc(handles)
  for a=1:3
    oUnits = get(handles.magFigureAxes{a},'Units');
    set(handles.magFigureAxes{a},'Units','Pixels');
    pos{a} = get(handles.magFigureAxes{a}, 'Position');
    set(handles.magFigureAxes{a},'Units',oUnits);
  end
  
  pt = get(handles.magFigure, 'CurrentPoint');
  xFig = pt(1,1);
  yFig = pt(1,2);
  ...
  crosshairOn=false;
  for a=1:3
    if (xFig>=pos{a}(1) & xFig<=pos{a}(1)+pos{a}(3) & yFig>=pos{a}(2) & yFig<=pos{a}(2)+pos{a}(4))
      pt = get(handles.magFigureAxes{a},'CurrentPoint');
      pt1 = round(pt(1,1) - 0.5);
      pt2 = round(pt(1,2) - 0.5);
      if pt1 < 1 || pt2 < 1, continue; end
      
      switch a
        case 1
          X=pt1;
          Y=str2double(get(handles.magFigureEdit{2},'String'));
          Z=handles.data.iheader.dim(3)+1-pt2;
        case 2
          X=str2double(get(handles.magFigureEdit{1},'String'));
          Y=pt1;
          Z=handles.data.iheader.dim(3)+1-pt2;
        case 3
          X=pt1;
          Y=handles.data.iheader.dim(2)+1-pt2;
          Z=str2double(get(handles.magFigureEdit{3},'String'));
        otherwise
          error('Bad point!');
      end
      if (handles.FlipDisplay)
      ...
      if all([X Y Z] <= handles.data.iheader.dim)
        crosshairOn=true;
      end
    end
  end
  ...
  if crosshairOn
    set(handles.magFigure, 'Pointer', 'cross');
    set(handles.magFigureStatus,'String',sprintf('Mouse: %d, %d, %d',X,Y,Z));
  else
    set(handles.magFigure, 'Pointer', 'arrow');
    set(handles.magFigureStatus,'String','');
  end
  
function MagMouseMvmt_Callback(hObject, eventdata, handles)
  handles=guidata(handles.WFU_PickAtlas); %get the actual handles from the main window
  [X Y Z] = GetMagMouseLoc(handles);

function MagMouseDown_Callback(hObject, eventdata, handles)
  handles=guidata(handles.WFU_PickAtlas); %get the actual handles from the main window
  [X Y Z] = GetMagMouseLoc(handles);
  set(handles.txtCubeX, 'string', num2str(X));
  set(handles.txtCubeY, 'string', num2str(Y));
  set(handles.txtCubeZ, 'string', num2str(Z));
  cmdGo1_Callback([], [], handles, [])
  

% --- Executes on button press in pushbutton25.
function ITKSnap_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton25 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global WFU_LOG;
if isempty(handles.itksnaptempdir) || exist(handles.itksnaptempdir,'dir') ~= 7
  handles.itksnaptempdir=tempname;
end
if exist(handles.itksnaptempdir,'dir') ~=7
  mkdir(handles.itksnaptempdir);
end

t1=fullfile(handles.data.atlas_toolbox,handles.data.SelectedAtlasType.subdir,handles.data.SelectedAtlasType.dispimage);
if exist(wfu_modify_filename(handles.data.SelectedAtlasType.dispimage,'dir',handles.itksnaptempdir),'file') ~= 2
  t1=fullfile(handles.data.atlas_toolbox,handles.data.SelectedAtlasType.subdir,handles.data.SelectedAtlasType.dispimage);
  wfu_copy_image(t1,handles.itksnaptempdir);
end
...
if isempty(handles.WorkList)
  ...
  atlasInUse=unique([handles.WorkList(:).Atlas]);
  segment=handles.Atlas(atlasInUse(1)).Iheader.fname;
  for a=2:length(atlasInUse)
    if ~strcmp(segment,handles.Atlas(atlasInUse(a)).Iheader.fname)
      warndlg('Cannot show more than one region map in ITKSnap at a time')
      segment=[];
    end
  end
  if ~isempty(segment)
    wfu_copy_image(segment,handles.itksnaptempdir);
    labels=wfu_modify_filename(segment,'ext','itksnap');
    if exist(labels,'file') == 2
      copyfile(labels,handles.itksnaptempdir,'f');
    else
      warning(sprintf('No ITKSnap labels for %s\n',segment));
      labels=[];
    end
  end
end

if ~isempty(t1),      t1=     wfu_modify_filename(t1,'dir',handles.itksnaptempdir);      end
if ~isempty(segment), segment=wfu_modify_filename(segment,'dir',handles.itksnaptempdir); end
if ~isempty(labels),  labels= wfu_modify_filename(labels,'dir',handles.itksnaptempdir);  end

if ~isempty(labels)
  labelStruct=wfu_read_itksnap_labels(labels);

  selectedLabels=zeros(1,length(handles.WorkList));
  for a=1:length(handles.WorkList)
    selectedLabels(a)=handles.Atlas(handles.WorkList(a).Atlas).Region(handles.WorkList(a).Region).SubregionValues(handles.WorkList(a).Subregion);
%For checking...
%    fprintf('%s\n',handles.Atlas(handles.WorkList(a).Atlas).Region(handles.WorkList(a).Region).SubregionNames{handles.WorkList(a).Subregion});
  end;
  maxLabel=max(length(labelStruct),max(selectedLabels));
  unselectedLabels=find(~ismember([1:maxLabel],selectedLabels));

  for a=1:length(selectedLabels)
    nidx=selectedLabels(a);
    labelStruct(nidx).VIS=1;
    labelStruct(nidx).MESH=1;
  end

  for a=1:length(unselectedLabels)
    nidx=unselectedLabels(a);
    labelStruct(nidx).VIS=0;
    labelStruct(nidx).MESH=0;
  end
  
  wfu_write_itksnap_labels(labelStruct,labels);
  
  
  cmd=wfu_pickatlas_itksnap;
  if ~isempty(t1),     cmd=sprintf('%s -g %s',cmd,t1);      end
  if ~isempty(segment) cmd=sprintf('%s -s %s',cmd,segment); end
  if ~isempty(labels)  cmd=sprintf('%s -l %s',cmd,labels);  end
  cmd=sprintf('%s &',cmd);
  WFU_LOG.info('Starting ITKSnap: %s',cmd);
  system(cmd);
end
guidata(handles.WFU_PickAtlas, handles);
...
Revision 3.7  2015/02/23 14:05:55  fmri
removed try/catch fileparts mess

Revision 3.6  2015/02/12 16:30:24  bwagner
Allow SPM12

Revision 3.5  2014/05/16 18:57:12  bwagner
Added sliders to MAG window

Revision 3.4  2014/01/31 20:08:47  bwagner
ITKSnap Button

Revision 3.3  2014/01/28 15:56:00  bwagner
Modified MAG into tri-plane view

Revision 3.2  2013/10/15 20:16:03  bwagner
Added Mag button

Revision 3.1  2013/04/05 14:18:22  bwagner
Fixed missing green dot

Revision 3.0  2011/10/20 14:22:46  bwagner
Moved to production from WFU_Development

Revision 1.18  2011/10/10 16:58:45  bwagner
