function [SPM,xSPM] = wfu_spm_getSPM(varargin)
% [SPM,xSPM] = wfu_spm_getSPM(xSPM,SPM);
% Modified for use with wfu_results_viewer and to allow PA from results
% interface.
%
...
% .k        - extent threshold {voxels}
% .thresDesc - description of height threshold (string)
% ADDED BCW 2010-08-25 
% .roi      - The file to mask by (empty will bypass PA, file 
%             will apply PA ROI.)
%
...
% $Id: wfu_spm_getSPM8.m,v 3.1 2015/02/12 16:30:56 bwagner Exp $
...
%--------------------------------------------------------------------------
xSPM=struct(); %default value (BCW 2010-08-25)
skipChecks=false; %used so that when a derived SPM struct is passed, several unneeded checks are skipped. (BCW 2010-08-27)
if nargin == 1
    xSPM = varargin{1};
elseif nargin == 2
    xSPM = varargin{1};
...
%BCW 2010-08-25 Get ROI
if ~isfield(xSPM,'roi')
  xSPM.roi=[];
end
if ~isfield(xSPM,'u') || isempty(xSPM.u) %then this is not a batch job
	if spm_input('ROI Analysis','+1','y/n',[1,0],2)
      ...
      [a b c d] = spm_fileparts(wfu_atlas_filename); %remove pesky `,1` from end of filename
      xSPM.roi=[a filesep b c];
    else	
      atlas_mask_filename=[swd filesep 'atlas_mask_file.nii'];
      [wfu_atlas_region,wfu_atlas_mask,xSPM.roi] = wfu_pickatlas(atlas_mask_filename);
    end
	end
end
...
% %J Maldjian 8-6-02
% %BCW modified 2001-08-25
% %-----------------------------------------------------------------------
% 
% if ~isempty(xSPM.roi)
%   fprintf('%s%30s',repmat(sprintf('\b'),1,30),'...Atlas ROI analysis');
% 	SPACE='I';
% 	D     = spm_vol(xSPM.roi);
%   if any(D.mat(:)~=SPM.VM.mat(:))
%     fprintf('%s%30s',repmat(sprintf('\b'),1,30),'...Reslice mask to SPM');
%     P = {SPM.VM.fname,xSPM.roi};
%     flags.mean=0;
%     flags.hold = 0;
%     flags.which=1;
%     flags.mask=0;
%     spm_reslice(P,flags);
%     [a b c d]=spm_fileparts(xSPM.roi);
%     xSPM.roi=fullfile(a,['r' b c]);
%   	D     = spm_vol(xSPM.roi);
%     if any(D.mat(:)~=SPM.VM.mat(:))
%       error('ROI reslice failed');
%     else
%       fprintf('%s%30s',repmat(sprintf('\b'),1,30),'...Continue Atlas ROI analysis');
%     end
%   end
% 	tM    = D.mat \ SPM.xVol.M;
% 	allowableShear=10^-4;
% 	if any(tM([2:5,7:10,12]))
% 		shearPoints=tM([2:5,7:10,12]);
% 		if any(find(shearPoints > allowableShear))
...
% 		else
% 			beep;
% 			warning(sprintf('Rotation/Shear detected but it is within the allowable range (0 to %f).  Maximum detected rotatation/shear is %f.',allowableShear,max(shearPoints)));
% 		end			
% 	end
...
%	XYZ   = D.mat \ [XYZmm; ones(1, size(XYZmm, 2))];
	j     = find(spm_sample_vol(D, XYZ(1,:), XYZ(2,:), XYZ(3,:),0) > 0);
  umXYZmm = SPM.xVol.M(1:3,:)*[SPM.xVol.XYZ; ones(1, size(SPM.xVol.XYZ,2))];
  umXYZ   = D.mat \ [     umXYZmm; ones(1, size(    umXYZmm, 2))];
	%k means something else in this script
  k2     = find(spm_sample_vol(D, umXYZ(1,:), umXYZ(2,:), umXYZ(3,:),0) > 0);
	S     = length(k2);
	R     = spm_resels(FWHM,D,SPACE);
%  R=R .* [1 2/3 2/3 1]; %R correction removed in 4010
	Z     = Z(j);
	XYZ   = XYZ(:,j);
	XYZmm = XYZmm(:,j);
...
%   %recompute stats
%   if STAT ~= 'P'
%     [up, Pp] = spm_uc_peakFDR(0.05,df,STAT,R,n,Z,SPM.xVol.XYZ(:,k2),u);
%     uu      = spm_uc(0.05,df,STAT,R,n,S);
%   end
%   try % if STAT == 'T'
% %      V2R               = 1/prod(xSPM.FWHM(DIM>1));
%       [uc, Pc, ue] = spm_uc_clusterFDR(0.05,df,STAT,R,n,Z,SPM.xVol.XYZ(:,k2),V2R,u);
%   catch
%       uc                = NaN;
%       ue                = NaN;
%       xSPM.Pc           = [];
...
if ~isempty(xSPM.roi)
  %recompute stats
  if STAT ~= 'P'
    [up, Pp] = spm_uc_peakFDR(0.05,df,STAT,R,n,Z,umXYZ(1:3,k2),u);
    uu      = spm_uc(0.05,df,STAT,R,n,S);
  end
  try % if STAT == 'T'
%      V2R               = 1/prod(xSPM.FWHM(DIM>1));
      [uc, Pc, ue] = spm_uc_clusterFDR(0.05,df,STAT,R,n,Z,umXYZ(1:3,k2),V2R,u);
  catch
      uc                = NaN;
      ue                = NaN;
      xSPM.Pc           = [];
