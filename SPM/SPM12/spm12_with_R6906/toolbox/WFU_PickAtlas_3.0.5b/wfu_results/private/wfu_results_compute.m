...
% $Revision: 1.9 $
  ...
  WFU_LOG.info('Entered function $Revision: 1.9 $');
...
% .thresDesc - description of height threshold (string)
  ...
  getSPMLoc1=fileparts(which('spm_getSPM'));
  getSPMLoc2=fileparts(which('wfu_spm_getSPM8'));
  getSPMLoc3=fileparts(which('wfu_spm_getSPM12'));
  if ~strcmp(getSPMLoc1,getSPMLoc2)
    WFU_LOG.error('locations of spm_getSPM and wfu_spm_getSPM8 are not the same!!! (%s != %s)',getSPMLoc1,getSPMLoc2);
  end
  if ~strcmp(getSPMLoc1,getSPMLoc3)
    WFU_LOG.error('locations of spm_getSPM and wfu_spm_getSPM8 are not the same!!! (%s != %s)',getSPMLoc1,getSPMLoc3);
  end
  
  [SPM xSPM]=spm_getSPM(xSPM,SPM);
...
Revision 1.9  2015/02/23 14:05:56  fmri
removed try/catch fileparts mess

Revision 1.8  2011/10/10 16:54:07  bwagner
