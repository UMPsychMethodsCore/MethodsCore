...
allsame = spm_input('Same mask in all extractions?', -1,'yes|no',[1,0],2);
...
if allsame == 0 
    for iExtraction = 1:nExtractions
        ...
        spmValsFiles{iExtraction}  = spm_get([0,Inf],'*.img',sprintf(['Pick T/Beta/Con' ...
                ' Image files extraction %d'],iExtraction),'./',0);
...
fprintf(fid,'Extraction NVoxels Mean Variance Mask\n');
for iExtraction = 1:nExtractions
