function varargout = wfu_list(varargin)
% FORMAT TabDat = wfu_list('List',SPM,hReg,[Num,Dis,Str],wfu_coords)
%                    Also added a 7th field to 'ListCluster'.  Arguments 4 to 6
%                    are junk variables.  I suggest using [] for them.
% FORMAT TabDat = wfu_list('listcluster',SPM,hReg,wfu_junk,wfu_junk,wfu_junk,wfu_coords)
%
% ----------------------------------------------------------------------------
% Modified:
%   2008-07-22 BCW   Added extra optional parameter to the 'List' sub-routine.
%                    If a 7th parameter (coordinates in mm), then calls to SPM
%                    display functions (spm_figure, etc.) are disabled.
%                    This has been done to enable this routine to work with
%                    wfu_results_ui.  The Optional Num, Dis, Str arguments
%                    may be set to null or [] for default SPM paramter to
%                    take effect when a 7th parameter is used.
%   2009-05-13 BCW   Updated to SPM8 version
%   2010-08-26 BCW   Updated to SPM8v4010
% ----------------------------------------------------------------------------
% Display and analysis of SPM{.}
% FORMAT TabDat = wfu_list('List',SPM,hReg,[Num,Dis,Str])
% Summary list of local maxima for entire volume of interest
% FORMAT TabDat = wfu_list('ListCluster',SPM,hReg,[Num,Dis,Str])
...
%                           ----------------
%
% FORMAT wfu_list('TxtList',TabDat,c)
...
%                           ----------------
%
% FORMAT wfu_list('SetCoords',xyz,hAx,hC)
...
%
% wfu_list characterizes SPMs (thresholded at u and k) in terms of
...
% $Id: wfu_list.m,v 1.2 2010/08/30 18:46:21 bwagner Exp $
...
%==========================================================================
% FORMAT TabDat = wfu_list('list',SPM,hReg)
...
if nargin < 3	
    ...
    %----------------------------------------------------------------------
    if length(varargin) > 3 & ~isempty(varargin{4}) & ~isempty(varargin{5}) %BCW - isempty part of mods
        Num    = varargin{4};         % number of maxima per cluster
        Dis    = varargin{5};         % distance among clusters (mm)
    else
      if isempty(varargin{4}) %BCW
        Num    = 3;
      end
      if isempty(varargin{5}) %BCW
        Dis    = 8;
      end
    end
    
    %BCW vars needed for printResultsTable
    TabDat.Num=Num;
    TabDat.Dis=Dis;
    
    if length(varargin) > 5 & ~isempty(varargin{6}) %BCW - isempty part of mods
        Title  = varargin{6};
    else
    ...
    %======================================================================
%-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
% This is all gui, don't use it if wfu_use_spm_ui is set
% BCW 2008-07-17
if wfu_use_spm_ui
%-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
...
%finding L can take a VERY long time - BCW
%    [N Z XYZ A L] = spm_max(zscores,varargin{2}.XYZ);
    ...
    if STAT~='P'
        if NONSTAT
            %Will need L here...so recompute spm_max - BCW
            [N Z XYZ A L] = spm_max(zscores,varargin{2}.XYZ);
            K     = zeros(c,1);
            for i = 1:c
...
if wfu_use_spm_ui
    ...
    %======================================================================
    % FORMAT TabDat = wfu_list('listcluster',SPM,hReg)
    % FORMAT TabDat = wfu_list('listcluster',SPM,hReg,wfu_junk,wfu_junk,wfu_junk,wfu_coords)

        wfu_pointers(gcf,'Watch');
...
if wfu_use_spm_ui
        ...
        %------------------------------------------------------------------
        if wfu_use_spm_ui
        	varargout = {wfu_list('list',SPM,hReg,Num,Dis)};
        else
        	varargout = {wfu_list('list',SPM,hReg,Num,Dis,[],xyzmm)};
        end
    ...
    %======================================================================
        % FORMAT wfu_list('SetCoords',xyz,hAx,hReg)
