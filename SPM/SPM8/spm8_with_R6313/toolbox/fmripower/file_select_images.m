function varargout = file_select_images(dirtype)
%@TODO - this is candidate for speedy deletion
...
%Make filter for .img, .img.gz, .nii and .nii.gz
  ...
  image_no=cellfun('isempty', regexp(all_names, 'img.gz'))+ ...
           cellfun('isempty', regexp(all_names, 'nii.gz'))+ ...
           cellfun('isempty', regexp(all_names, '.img'))+ ...
           cellfun('isempty', regexp(all_names, '.nii')); 

file_names=all_names(dir_ind==0 & image_no<4);
