function spm_image_pow_individual(op,varargin)
...
if nargin == 0,
	...
	spm_image_pow_individual('init',P);
	return;
end;
...
if strcmp(op,'repos'),
	...
	% spm_orthviews('MaxBB');
	spm_image_pow_individual('zoom_in');
	spm_image_pow_individual('update_info');
...
if strcmp(op,'shopos'),
	...
	if isfield(st,'mp'),
		...
		if any(findobj(fg) == st.mp),
            ...
            set(st.in1,'String',sprintf('%g',...
                 spm_sample_vol(st.power,pos(1),pos(2),pos(3),st.hld)));
	    set(st.in2,'String',sprintf('%g',spm_sample_vol(st.mean_sd, ...
		      pos(1),pos(2),pos(3),st.hld)));	
	     set(st.in3,'String',sprintf('%g',...
                 spm_sample_vol(st.mean,pos(1),pos(2),pos(3),st.hld)));
	    set(st.in4,'String',sprintf('%g',spm_sample_vol(st.sd,pos(1), ...
			  pos(2),pos(3),st.hld)));
...
if strcmp(op,'addblobs'),
	...
	for i=1:nblobs,
		...
		set(st.blobber,'String','Remove Blobs','Callback','spm_image_pow_individual(''rmblobs'');');
	end;
...
if strcmp(op,'rmblobs'),
	...
	set(st.blobber,'String','Add Blobs','Callback','spm_image_pow_individual(''addblobs'');');
end;
...
if strcmp(op,'reorient'),
	...
	if sum((tmp(:)-st.vols{1}.mat(:)).^2) > 1e-8,
		spm_image_pow_individual('init',st.vols{1}.fname);
	end;
	return;
end;
...
if strcmp(op,'init'),
    ...
    %start with power(top left)
     spm_orthviews('Image', P, [0.025, 0.55, 0.45, 0.55]);
     spm_orthviews('addimage',1, P2);

    %I'm adding mean in SD units (top right)
    spm_orthviews('Image', P, [0.525 0.55 .45 0.55]);
    spm_orthviews('addimage',2, P3);

    %I'm adding mean  (bottom left)
    spm_orthviews('Image', P, [0.025 0.125 .45 0.55]);
    spm_orthviews('addimage',3, P4);

    %I'm adding sd  (bottom right)
    spm_orthviews('Image', P, [0.525 0.125 .45 0.55]);
    spm_orthviews('addimage',4, P5);
    ...
    spm_orthviews('MaxBB');
    st.callback = 'spm_image_pow_individual(''shopos'');';
    ...
    uicontrol(fg,'Style','Text', 'Position',[65 830 130 20].*WS,...
              'String', 'Power (%)', ...
          'FontSize', spm('FontSize',13), 'Backgroundcolor', [1 1 1]);
    uicontrol(fg,'Style','Text', 'Position',[320 830 230 20].*WS,'String','Mean  (SD units)',...
              'FontSize', spm('FontSize',13), 'Backgroundcolor', [1 1 1]);
    %Power L and R
    uicontrol(fg, 'Style','Text', 'Position', [18 560 10 13].*WS, 'String', 'L', ...
    		'FontSize', spm('FontSize',10), 'Backgroundcolor', [ 0 0 0 ], 'ForegroundColor', [1 1 1]);
    
    
    
    uicontrol(fg,'Style','Text', 'Position',[75 475 130 20].*WS,...
             'String','Mean', 'tooltipstring', ...
             'Means have not been scaled to % change from baseline',...
          'FontSize', spm('FontSize',13), 'Backgroundcolor', [1 1 1]);
    uicontrol(fg,'Style','Text', 'Position',[330 475 230 20].*WS,'String','SD',  ...
               'FontSize', spm('FontSize',13), 'Backgroundcolor', [1 1 1], ...
                'tooltipstring', 'SDs have not been scaled to % change from baseline'  );
    ...
    %uicontrol(fg,'Style','PushButton', 'Position',[15 116 170 006].*WS,...
    %	'Callback',...
    %        'spm_orthviews(''Reposition'',[0 0 0]);','ToolTipString','move crosshairs to origin');

    uicontrol(fg,'Style','Text', 'Position',[130 105 60 030].*WS,'String','mm:', ...
               'Fontsize', spm('FontSize',12), 'HorizontalAlignment', 'left');
    ...
    st.mp = uicontrol(fg,'Style','edit', 'Position',[240 105 200 030].*WS,...
                    'String','','Callback','spm_image_pow_individual(''setposmm'')',...
    ...
    st.vp = uicontrol(fg,'Style','edit', 'Position',[240 70 200 030].*WS,...
                     'String','','Callback','spm_image_pow_individual(''setposvx'')',...
                      'ToolTipString','move crosshairs to voxel coordinates', ...
                       'Fontsize', spm('Fontsize',14));
    st.in1 = uicontrol(fg,'Style','Text', 'Position',[210 655  85 020].*WS, ...
               'String','', 'FontSize', spm('Fontsize',14),'BackgroundColor', [1,1,1]);
    st.in2 = uicontrol(fg,'Style','Text', 'Position',[500 655  85 020].*WS, ...
               'String','', 'Fontsize', spm('Fontsize',14), 'BackgroundColor', [1,1,1]);
    st.in3 = uicontrol(fg,'Style','Text', 'Position',[210 290  85 020].*WS, ...
               'String','', 'FontSize', spm('Fontsize',14),'BackgroundColor', [1,1,1]);
    st.in4 = uicontrol(fg,'Style','Text', 'Position',[500 290  85 020].*WS, ...
               'String','', 'Fontsize', spm('Fontsize',14), 'BackgroundColor', [1,1,1]);
