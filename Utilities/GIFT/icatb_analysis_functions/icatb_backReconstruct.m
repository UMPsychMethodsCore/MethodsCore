...
if (~strcmpi(algorithmName, 'iva-gl'))
    ...
    if (~strcmpi(backReconType, 'spatial-temporal regression') && ~strcmpi(backReconType, 'moo-icar'))
...
if (~strcmpi(backReconType, 'spatial-temporal regression') && ~strcmpi(backReconType, 'moo-icar'))
    ...
    if (~strcmpi(deblank(icaStr(sesInfo.algorithm, :)), 'iva-gl'))
    ...
    for nSet = dataSetsToRun
        ...
        % Read data
        data = icatb_read_data(str2mat(sesInfo.inputFiles(nSet).name), [], sesInfo.mask_ind);
        ...
        compSet = struct(compSetFields{1}, spatial_maps, compSetFields{2}, tc);
        
        clear tc spatial_maps;
