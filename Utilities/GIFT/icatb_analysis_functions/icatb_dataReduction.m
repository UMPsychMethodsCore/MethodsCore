...
for j = reductionStepsToRun
    ...
    if(j > 1)
        ...
        if (strcmpi(pcaType, 'standard'))
            %% Compute pca along voxel dimension
            statusPCAVoxDim = computePCAVoxelDims(sesInfo, preproc_type, gpca_opts{j}.pca_opts.storage, gpca_opts{j}.pca_opts.precision);
            if (statusPCAVoxDim == 1)
                break;
            end
        end
        ...
        for i = dataSetsToRun %1:numOfGroupsAfter
            ...
            if(j == 1)
                ...
                if (strcmpi(pcaType, 'standard'))
                    ...
                    if (conserve_disk_space ~= 1)
                        % Calculate pca step
                        [pcasig, dewhiteM, Lambda, V, whiteM] = icatb_calculate_pca(data, numOfPC, 'remove_mean', 0, 'whiten', 1);
                    else
                        [V, Lambda] = icatb_calculate_pca(data, numOfPC, 'remove_mean', 0, 'whiten', 0);
                    end
...
if ((sesInfo.numReductionSteps == 1) && strcmpi(algoName, 'iva-gl'))
    try
        conserve_disk_space = sesInfo.conserve_disk_space;
    catch
    end
...
function status = computePCAVoxelDims(sesInfo, preproc_type, storage, precisionType)
%% Compute PCA along voxel dimension
%

status = 0;

algoStr = cellstr(icatb_icaAlgorithm);
algoVal = sesInfo.algorithm;
MT = sesInfo.userInput.numOfPC1*sesInfo.numOfSub*sesInfo.numOfSess;
voxels = length(sesInfo.mask_ind);
...
%% Add covariance matrix across voxel dimension
for j = 1:sesInfo.numOfSub*sesInfo.numOfSess
    if (exist('VStacked', 'var'))
        pcasig = whitenData(sesInfo, preproc_type, j, VStacked, LambdaStacked);
    else
        pcain = [sesInfo.data_reduction_mat_file, '1-', num2str(j), '.mat'];       
        load(fullfile(outputDir, pcain), 'pcasig');
    end
    ...
    if (~strcmpi(storage, 'full'))
        if (j == 1)
            if (isa(pcasig, 'single'))
                cov_m = zeros(voxels*(voxels + 1)/2, 1, 'single');
            else
                cov_m = zeros(voxels*(voxels + 1)/2, 1);
            end
        end
        
        pcasig = pcasig';
        
        %% Add voxel covariance
        cov_m = compute_cov(cov_m, pcasig);
        
    else
        if (j == 1)
            if (isa(pcasig, 'single'))
                cov_m = zeros(voxels, voxels, 'single');
            else
                cov_m = zeros(voxels, voxels);
            end
        end
        % Add voxel covariance
        cov_m = cov_m + (pcasig*pcasig'/(voxels - 1));
    end
    
    clear pcasig;
end

%% Compute group PCA components
[pcasig, Lambda] = icatb_eig_symm(cov_m, voxels, 'num_eigs', sesInfo.numComp);
pcasig = pcasig*diag(1./std(pcasig));

pcain = [sesInfo.data_reduction_mat_file, num2str(sesInfo.numReductionSteps), '-1.mat'];
icatb_save(fullfile(outputDir, pcain), 'pcasig', 'Lambda');

pcasigN = pcasig;

clear pcasig;

dewhiteM = zeros(MT, sesInfo.numComp);
...
for j = 1:sesInfo.numOfSub*sesInfo.numOfSess
    ...
    startT = endT + 1;
    endT = endT + size(pcasig, 2);
    dewhiteM(startT:endT, :) = (pinv(pcasigN)*pcasig)';
    clear pcasig;
...
%% Save
pcain = [sesInfo.data_reduction_mat_file, num2str(sesInfo.numReductionSteps), '-1.mat'];
icatb_save(fullfile(outputDir, pcain), 'whiteM', 'dewhiteM', '-append');
...
function cov_m = compute_cov(cov_m, A)
...
function pcasig = whitenData(sesInfo, preproc_type, j, VStacked, LambdaStacked)
%% Whiten data
%
...
whiteM = sqrtm(diag(LambdaStacked(j, :))) \ VStacked(startT:endT, :)';
dat = icatb_read_data(sesInfo.inputFiles(j).name, [], sesInfo.mask_ind);
% Call pre-processing function
dat = icatb_preproc_data(dat, preproc_type, 0);
% Remove mean per timepoint
dat = icatb_remove_mean(dat, 0);
pcasig = dat*whiteM';
clear wM dat;
