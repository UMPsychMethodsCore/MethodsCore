...
if strcmpi(modalityType, 'fmri')
    tagsTobePlotted = {'prefix', 'files', 'estimate_components', 'numComp', 'autofill', 'algorithm', 'which_analysis'};
elseif strcmpi(modalityType, 'eeg')
    tagsTobePlotted = {'prefix', 'files', 'numComp', 'autofill', 'algorithm', 'which_analysis'};
else
...
algoIndex = strmatch('algoType', str2mat(inputText.tag), 'exact');
...
if numOfSub*numOfSess > 0
    ...
    compIndex = strmatch('numComp', str2mat(inputText.tag), 'exact');
    ...
    dataRedIndex = strmatch('numReductionSteps', str2mat(inputText.tag), 'exact');
    
    inputText(dataRedIndex).enable = 'off';
    
    numReductionSteps = 1;
    if (~strcmpi(deblank(icaStr(icaVal, :)), 'iva-gl') && ~strcmpi(deblank(icaStr(icaVal, :)), 'moo-icar') && ~strcmpi(deblank(icaStr(icaVal, :)), 'constrained ica (spatial)'))
        % Number of data reduction steps
        if(numOfSub == 1 && numOfSess == 1)
            numReductionSteps = 1;
            %elseif(numOfSub*numOfSess > 10)
            %    numReductionSteps = 3;
        elseif(numOfSub*numOfSess < 4)
            numReductionSteps = 2;
        else
    ...
    prefix1 = 'numOfPC1'; prefix2 = 'numOfPC2'; prefix3 = 'numOfPC3';
    
    % get the index of the PC1, PC2, PC3
    PC1Index = strmatch(prefix1, str2mat(inputText.tag), 'exact');
    PC2Index = strmatch(prefix2, str2mat(inputText.tag), 'exact');
    PC3Index = strmatch(prefix3, str2mat(inputText.tag), 'exact');
    ...
    if numReductionSteps == 1
        ...
        %% PC3
        inputText(PC3Index).promptString = 'Number Of PC (Step 3)';
        inputText(PC3Index).enable = 'inactive';
        sesInfo.userInput.numOfPC3 = 0;
        inputText(PC3Index).answerString = num2str(sesInfo.userInput.numOfPC3);
    ...
    elseif numReductionSteps == 3
        ...
        %%% PC2
        inputText(PC2Index).promptString = 'Number Of PC (Step 2)';
        inputText(PC2Index).enable = 'on';
        if ~isfield(sesInfo.userInput, 'numOfPC2')
            
            sesInfo.userInput.numOfPC2 = PCBefore; %sesInfo.userInput.numComp;
        else
            if isempty(sesInfo.userInput.numOfPC2)
                sesInfo.userInput.numOfPC2 = PCBefore; %sesInfo.userInput.numComp;
            end
        end
        inputText(PC2Index).answerString = num2str(sesInfo.userInput.numOfPC2);
        ...
        %%% PC3
        inputText(PC3Index).promptString = 'Number Of PC/IC (Step 3)';
        inputText(PC3Index).enable = 'inactive';
...
try
    ...
    if (strcmpi(tmpAlgStr{tmpAlgVal}, 'moo-icar')  || strcmpi(tmpAlgStr{tmpAlgVal}, 'constrained ica (spatial)'))
    ...
    if strcmp(getSubjects, 'yes')
        ...
        if isfield(sesInfo.userInput, algoTag)
            ...
            if (strcmpi(algoStr{algoVal}, 'iva-gl') || strcmpi(algoStr{algoVal}, 'moo-icar')  || strcmpi(algoStr{algoVal}, 'constrained ica (spatial)'))
        ...
        if isfield(sesInfo.userInput, whichAnalysisTag)
            ...
            whichAnalysisVal = lower(getfield(sesInfo.userInput, whichAnalysisTag));
        ...
        backReconIndex = strmatch(backReconTag, cellstr(str2mat(inputText.tag)), 'exact');
        ...
        preprocIndex = strmatch(preprocTag, cellstr(str2mat(inputText.tag)), 'exact');
        ...
        group_pcaIndex = strmatch(groupPCATag, cellstr(str2mat(inputText.tag)), 'exact');
        ...
        pcaIndex = strmatch(pcaTag, cellstr(str2mat(inputText.tag)), 'exact');
        if ~isempty(pcaIndex)
        ...
        if isfield(sesInfo.userInput, 'scaleType') %& isfield(sesInfo.userInput, 'convertToZ')
            ...
            adjustTag = 'scaleType';
            adjustIndex = strmatch(adjustTag, str2mat(inputText.tag), 'exact');
        ...
        if isfield(sesInfo.userInput, maskTag)
            ...
            maskIndex = strmatch(maskTag, str2mat(inputText.tag), 'exact');
...
tagsNeeded = {'numOfPC1', 'numOfPC2', 'numOfPC3', 'numReductionSteps'};
...
if numRedSteps == 3
    ...
    set(prompt3, 'string', 'Number Of PC/IC (Step 3)');
    set(PC3, 'enable', 'inactive', 'string', num2str(sesInfo.userInput.numComp));
...
elseif numRedSteps == 2
    ...
    set(prompt3, 'string', 'Number Of PC3');
    set(PC3, 'enable', 'inactive', 'string', '0');
...
try
    ...
    inputText = menuData.inputText;
    % mask index
    maskIndex = strmatch('maskFile', str2mat(inputText.tag), 'exact');
    % input text structure
...
try
    ...
    if (strcmpi(deblank(icaStr(icaVal, :)), 'iva-gl') || strcmpi(deblank(icaStr(icaVal, :)), 'moo-icar') || strcmpi(deblank(icaStr(icaVal, :)), 'constrained ica (spatial)'))
        ...
        if (sesInfo.userInput.numOfSub*sesInfo.userInput.numOfSess > 1) && (sesInfo.userInput.numReductionSteps==1)
    ...
    if (~isfield(sesInfo.userInput, 'preproc_type'))
        try
            preProcTag = strmatch('preproc_type', str2mat(inputText.tag), 'exact');
    ...
    gPCAVal = strmatch('group_pca_type', str2mat(inputText.tag), 'exact');
    if (~isempty(gPCAVal))
    ...
    if (~isfield(sesInfo.userInput, 'pcaType'))
        pcaTag = strmatch('pcaType', str2mat(inputText.tag), 'exact');
    ...
    if (~isfield(sesInfo.userInput, 'backReconType'))
        try
            backReconTag = strmatch('backReconType', str2mat(inputText.tag), 'exact');
    ...
    adjustTag = strmatch('scaleType', str2mat(inputText.tag), 'exact');
    ...
    elseif numReductionSteps == 2
        ...
        % Error check for PC2
        [currentVar] = check_var_integer(num2str(sesInfo.userInput.numOfPC2));
        
        if isempty(currentVar)
            error(['Not a valid Number for PC (Step 2). Please click on SetupICA-Defaults', ...
                ' menu to see Number of PC (step 2).']);
        end
        
        % Check PC1
        if sesInfo.userInput.numOfPC1 < 2
            error(['Select Number for PC (Step 1) to be more than or equal to 2. Please click on', ...
                ' SetupICA-Defaults menu to see Number of PC (step 1).']);
        end
        
        % Check PC2
        if sesInfo.userInput.numOfPC2 < 2
            error(['Select Number for PC (Step 2) to be more than or equal to 2. Please click on SetupICA-Defaults ', ...
                'menu to see Number of PC (step 2).']);
        end
...
if (strcmpi(allAlgorithms{sel_algorithm}, 'semi-blind infomax'))
...
matchedInd = strmatch(lower(allAlgorithms{sel_algorithm}), lower({'Infomax', 'Fast ICA', 'SDD ICA', 'Semi-blind Infomax', 'Constrained ICA (Spatial)', 'FBSS', 'IVA-GL'}), 'exact');
...
if (strcmpi(allAlgorithms{sel_algorithm}, 'constrained ica (spatial)') || strcmpi(allAlgorithms{sel_algorithm}, 'MOO-icar'))
    
    if ~strcmpi(allAlgorithms{sel_algorithm}, 'moo-icar')
        ...
        spatial_references = icatb_selectEntry('title', 'Select aggregate images from group ica ...', ...
            'typeEntity', 'file', 'typeSelection', 'multiple', 'filter', '*agg*comp*.img;*agg*comp*.nii', 'fileType', 'image');
        if isempty(spatial_references)
...
if (strcmpi(icaTypes{icaVal}, 'moo-icar') || strcmpi(icaTypes{icaVal}, 'constrained ica (spatial)'))
    set(numCompH, 'enable', 'inactive');
    set(whichAnalysisH, 'enable', 'inactive');
end
