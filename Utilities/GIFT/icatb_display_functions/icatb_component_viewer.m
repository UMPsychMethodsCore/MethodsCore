...
for ncomp = compNumbers
    ...
    count = count + 1;
    [tc, dynamicrange(count), fALFF(count)] = compute_spectra(squeeze(timecourses(:, :, count)), TR, freq_limits);
    if (count == 1)
    ...
    H = icatb_orth_views(deblank(compFiles(ncomp, :)), 'structfile', anatomical_file, 'image_values', image_values, 'convert_to_zscores', convert_to_zscores, 'threshold', threshold, 'set_to_max_voxel', 1, 'tc', tc, 'labels', ...
        '', 'fig_title', [viewing_set, ' ', icatb_returnFileIndex(ncomp)]);
...
function [tc, dynamicrange, fALFF] = compute_spectra(timecourses, TR, freq_limits)
%% Compute spectra
