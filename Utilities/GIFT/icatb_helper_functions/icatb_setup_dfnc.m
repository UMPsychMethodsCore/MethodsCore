...
drawnow;
structFile = icatb_rename_4d_file(deblank(dfncInfo.userInput.compFiles(1, :)));
structFile = deblank(structFile(1, :));
if (~exist(icatb_parseExtn(structFile), 'file'))
    structFile = deblank(dfncInfo.userInput.compFiles(1, :));
end
...
for nC = 1:size(dfncInfo.userInput.compFiles, 1)
    
    tmp = icatb_spm_read_vols(icatb_spm_vol(dfncInfo.userInput.compFiles(nC, :)));
    ...
    if (nC == 1)
        compData = zeros(size(tmp, 1), size(tmp, 2), dfncInfo.userInput.numICs);
    end
    
    [dd, inds] = max(tmp(:));
    
    %maxCompVal = max ([maxCompVal, max(abs(tmp(:)))]);
...
% if (dfncInfo.userInput.numOfSub < 2)
