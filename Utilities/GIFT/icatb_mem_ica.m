...
if (~strcmpi(icaAlgo, 'iva-gl') && ~strcmpi(icaAlgo, 'moo-icar') && ~strcmpi(icaAlgo, 'constrained ica (spatial)'))
...
elseif strcmpi(icaAlgo, 'iva-gl')
    initial_num_bytes = [max([(voxels*time_points*2 + time_points^2), voxels*numOfPC1*numOfSub*numOfSess]), max([(voxels*time_points + voxels*numOfPC1 + numOfPC1*numOfPC1 + numOfPC1*time_points), voxels*numOfPC1*numOfSub*numOfSess])];
else
    initial_num_bytes = [voxels*time_points*2 + (time_points*time_points)^2, voxels*time_points*2 + (time_points*time_points)^2];
end
...
if (numDataReductionSteps == 2)
    ...
    if (voxels > tmp_datasets)
        ...
        % Covariance storage (full)
        cov_bytes = 2*(voxels^2);
        cov_bytes_packed = voxels*(voxels + 1);
        % Stacked data
        stacked_data_bytes = numOfPC1*voxels*2;
    ...
    for nA = 1:length(analysisTypes)
        ...
        for nT = 1:length(stack_data)
            ...
            if (strcmpi(analysisTypes{nA}, 'standard'))
                ...
                if (numDataReductionSteps == 2)
                    stacked_data_bytes = numOfSub*numOfSess*numOfPC1*voxels*2;
                end
