...
if local                                    %local efficiency
    E=zeros(N,1);                           %local efficiency
     ...
     Gw(eye(size(Gw))~=0)=0;                %@Yu: set self connection to zero, avoid counting itself as a neighbor
    for u=1:N
        V=find(Gw(u,:));                    %neighbors
        k=length(V);                        %degree
        if k>=2;                            %degree must be at least two
            e=( distance_inv_wei(Gl(V,V)) .* (Gw(V,u)*Gw(u,V)) ).^(1/3) ; % equation from third reference(comment from @Yu)
