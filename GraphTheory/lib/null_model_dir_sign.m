...
if wei_freq<0 || wei_freq>1
    error('wei_freq must range between 0 and 1.')
end
if wei_freq && wei_freq<1 && nargin('randperm')==1
    warning('wei_freq may only equal 1 or 0 in older (<2011) versions of MATLAB.')
    wei_freq=1;
end
...
for s=[1 -1]
    ...
    switch wei_freq
        ...
        case 0
            [dum Oind]=sort(P(Lij));                    %get indices of Lij that sort P
            W0(Lij(Oind)) = s*Wv;                       %assign corresponding sorted weight at this index
        otherwise
