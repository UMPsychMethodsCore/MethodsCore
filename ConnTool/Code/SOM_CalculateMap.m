...
% Copyright 2005
%
% function results = SOM_CalculateMap(theData,nIter,mapSwitch,nSOM,[SelfOMap],[slot])
%
% theData   = theData(nSpace,nTime);
% nSOM      = number of basis functions in the SOM
% nIter     = number of iterations.
% mapSwitch = vertex layout   'grid' = square layout
...
%                                       2 = weight by st-dev
%      SOM.silent                       0 = verbose
%                                       1 = non-verbose
%
%
%      SOM.maskThresh                = 12.5% - by default.
%
...
function results = SOM_CalculateMap(theData,nIter,map_switch,nSOM,SelfOMap,slot)
...
% Determine whether a sphere or grid for the SOM examplar organization.

if length(map_switch) == 6 & lower(map_switch) == 'sphere'
    if ~isfield(SOM,'sphereIter')
        SOM.sphereIter = 1000;
    end
    v = SOM_Sphere_Make(nSOM, SOM.sphereIter);
    results.v2 = v(:,1:3);
    results.vn = v;
    v = v(:,1:3);
    SOM.sphere_option = 1;
elseif length(map_switch) == 4 & lower(map_switch) == 'grid'
    ...
    v = zeros(nSOM,3);
    [x,y] = meshgrid(1:sqrt(nSOM),1:sqrt(nSOM));
    v(:,1) = reshape(x,[nSOM,1]);
    v(:,2) = reshape(y,[nSOM,1]);
    SOM.sphere_option = 0;
else
    fprintf('Not a valid map switch option, valid options are:\n');
    fprintf('  ''Sphere'' & ''Grid''\n');
end
...
% This is if we have square grid, else the info is meaningless
% if we have spherical space.
...
if SOM.sphere_option  %sphere distance map
...
while iter < nIter
    ...
    for iSOM = 1:nSOM
        ...
        if length(dI) > 0
            ...
            %
            pV = sum(pV_init(:,dI),2);
            %pV = sum(pV_init(:,dI),2)/sum(SOMMem{slot}.Weights(dI,1));
            %pV = SOM_UnitNormMatrix(pV,1);
            %-------------------Ben changed 6/13/07-----------------------
            if SOM.OldMethod == 0
    ...
    % Update the SOM.
    %dSelfOMap = dSelfOMap/(nSpace/nSOM);
