...
for iRUN = 1:length(data.run)
    ...
    if isfield(data.run(iRUN),'nTIME') == 0
        ...
        if data.run(iRUN).nTIME ~= nTIME
            SOM_LOG(sprintf('STATUS : nTIME = min([%d %d])',data.run(iRUN).nTIME,nTIME));
            data.run(iRUN).nTIME = min([data.run(iRUN).nTIME nTIME]);
        end
    end
    
    if isfield(data.run(iRUN),'despikeVector') && ~isempty(data.run(iRUN).despikeVector)
        SOM_LOG(sprintf('STATUS : Found despikeVector for run %d',iRUN));
        if length(data.run(iRUN).despikeVector) ~= data.run(iRUN).nTIME
            SOM_LOG(sprintf('FATAL : Specified despikeVector of length %d for run %d does not match %d',length(data.run(iRUN).despikeVector),iRUN,data.run(iRUN).nTIME));
            return
        end
    end
    
    if isfield(data.run(iRUN),'censorVector') && ~isempty(data.run(iRUN).censorVector)
        ...
        if length(data.run(iRUN).censorVector) ~= data.run(iRUN).nTIME
            SOM_LOG(sprintf('FATAL : Specified censorVector of length %d for run %d does not match %d',length(data.run(iRUN).censorVector),iRUN,data.run(iRUN).nTIME));
            return
        end
    end
    ...
    if isfield(data.run(iRUN),'MotionParameters') == 1
        if size(data.run(iRUN).MotionParameters,1) ~= data.run(iRUN).nTIME
            ...
            if ~isempty(data.run(iRUN).MotionParameters)  && size(data.run(iRUN).MotionParameters,1) < data.run(iRUN).nTIME
