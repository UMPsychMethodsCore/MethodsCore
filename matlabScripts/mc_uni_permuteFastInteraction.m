function [stat] = mc_uni_permuteFastInteraction(data, design,permcol,intcols,vals)
%
% This is a version of mc_uni_permuteFast tweaked to allow for modeling interactions with
% the predictor of interest
%
% The design variable that you pass it should not include the interaction, this will get tacked on later.
%
...
% Build full design
if numel(unique(design(:,permcol)))==2 % if categorical, reset variable to binary
    design(:,permcol) = sign(design(:,permcol));
    design(:,permcol) == 0;
end

for iTerm = 1:numel(intcols); % tack on interactions to the end
    design = [design design(:,permcol) .* design(:,intcols(iTerm))];
end
