function [SPM,xSPM] = mike_getSPM(varargin)
...
SPMid = 'SPM2: mike_getSPM (v2.51)';
%spm_help('!ContextHelp',mfilename)

if ((length(varargin) > 4 || length(varargin)<4) && length(varargin) > 0) 
  error('Too many parameters to mike_getSPM');
end
...
if (isempty(m_path))
  swd = input('Path to SPM.mat file: ','s');
else
  swd = m_path;
end
...
%-Get contrasts
%-----------------------------------------------------------------------
%[Ic,xCon] = spm_conman(xX,xCon,'T&F',Inf,'	Select contrasts...',' for conjunction',1)
if (isempty(m_contrast))
  Ic = input('Contrast number: ');
else
  Ic = m_contrast;
end
...
if (isempty(m_extent))
  k = input('Extent Threshold (voxels): ');
else
  k = m_extent;
end
