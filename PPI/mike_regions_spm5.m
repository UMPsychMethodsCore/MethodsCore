function [Y,xY] = mike_regions_spm5(xSPM,SPM,hReg,varargin)
...
if ~isfield(xY,'def')
	...
	%		{'sphere','box','cluster','mask'});
        if (isempty(m_def))
            xY.def = input('image, sphere, box: ','s');
        else
            xY.def = m_def;
        end
...
switch xY.def
	...
	%---------------------------------------------------------------
	if ~isfield(xY,'spec')
		%xY.spec = spm_vol(spm_select(1,'image','Specify
                %Mask'));
                if (isempty(m_spec))
                    xY.spec = input('Specify image: ','s');
                else
                    xY.spec = m_spec;
                end
