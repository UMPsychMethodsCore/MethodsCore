function varargout = mike_results_ui(varargin)
% User interface for SPM/PPM results: Display and analysis of regional effects
% FORMAT [hReg,xSPM,SPM] = mike_results_ui
%
...
%                           ----------------
%
% In addition to setting up the results section, mike_results_ui.m sets
...
% body of the code.
%_______________________________________________________________________
% @(#)mike_results_ui.m	2.42 Karl Friston, Andrew Holmes 03/02/28
SCCSid = '2.42';
...
%
% mike_results_ui sets up and handles the SPM results graphical user
% unterface, initialising an XYZ registry (see spm_XYZreg.m) to co-ordinate
% locations between various location controls.
%
%_______________________________________________________________________
%
% FORMAT hReg = mike_results_ui('SetupGUI',M,DIM,xSPM,Finter)
...
%
% FORMAT mike_results_ui('DrawButts',hReg,DIM,Finter,WS,FS)
% Draw GUI buttons
% hReg    - handle of XYZ registry object
% DIM     - 3 vector of image X, Y & Z dimensions
...
%
% FORMAT hFxyz = mike_results_ui('DrawXYZgui',M,DIM,xSPM,xyz,Finter)
...
%
% FORMAT mike_results_ui('EdWidCB')
% Callback for editable XYZ control widgets
%
% FORMAT mike_results_ui('UpdateSPMval',hFxyz)
% FORMAT mike_results_ui('UpdateSPMval',UD)
...
%
% FORMAT xyz = mike_results_ui('GetCoords',hFxyz)
...
%
% FORMAT [xyz,d] = mike_results_ui('SetCoords',xyz,hFxyz,hC)
...
% NB: When using the results section, should use XYZregistry to get/set location
%
% FORMAT hFxyz = mike_results_ui('FindXYZframe',h)
...
%
% FORMAT hMP = mike_results_ui('LaunchMP',M,DIM,hReg,hBmp)
% Prototype callback handler for integrating MultiPlanar toolbox
%
% FORMAT mike_results_ui('Delete',h)
...
if (length(varargin) > 1)
  m_path = varargin{2};
  m_contrast = varargin{3};
  m_threshold = varargin{4};
  m_extent = varargin{5};
  [SPM,xSPM] = mike_getSPM(m_path, m_contrast, m_threshold, m_extent);
else
  [SPM,xSPM] = mike_getSPM;
end
...
hReg      = mike_results_ui('SetupGUI',M,DIM,xSPM,Finter);
...
%-----------------------------------------------------------------------
hFxyz      = mike_results_ui('DrawXYZgui',M,DIM,varargin{4},xyz,Finter);
spm_XYZreg('XReg',hReg,hFxyz,'mike_results_ui');

%-Set up buttons for results functions
%-----------------------------------------------------------------------
mike_results_ui('DrawButts',hReg,DIM,Finter,WS,FS);
...
hClear = uicontrol(Finter,'Style','PushButton','String','clear',...
	...
	'Callback',['mike_results_ui(''Clear''); ',...
...
hHelp  = uicontrol(Finter,'Style','PushButton','String','?',...
	...
	'Callback','spm_help(''mike_results_ui'')',...
	'Interruptible','on','Enable','on',...
...
%=======================================================================
% hFxyz = mike_results_ui('DrawXYZgui',M,DIM,xSPM,xyz,Finter)
...
hX   = uicontrol(Finter,'Style','Edit','String',sprintf('%.2f',xyz(1)),...
	...
	'Tag','hX',...
	'Callback','mike_results_ui(''EdWidCB'')');
...
if abs(xyz(d)-nxyz(d))>0
	...
	mike_results_ui('UpdateSPMval',UD)
end

%=======================================================================
case 'updatespmval'                            %-Update SPM value in GUI
%=======================================================================
% mike_results_ui('UpdateSPMval',hFxyz)
% mike_results_ui('UpdateSPMval',UD)
...
%=======================================================================
% xyz = mike_results_ui('GetCoords',hFxyz)
if nargin<2, hFxyz='Interactive'; else, hFxyz=varargin{2}; end
hFxyz     = mike_results_ui('FindXYZframe',hFxyz);
...
%=======================================================================
% [xyz,d] = mike_results_ui('SetCoords',xyz,hFxyz,hC)
if nargin<4, hC=0; else, hC=varargin{4}; end
if nargin<3, hFxyz=mike_results_ui('FindXYZframe'); else, hFxyz=varargin{3}; end
...
%=======================================================================
% hFxyz = mike_results_ui('FindXYZframe',h)
...
h4 = uicontrol(Finter,'Style','PopUp',...
	...
	'Callback','mike_results_ui(''PlotUiCB'')',...
	'Interruptible','on','Enable','on',...
...
set(hGraphUIbg,'UserData',...
	...
	'DeleteFcn','mike_results_ui(''Delete'',get(gcbo,''UserData''))')
...
%=======================================================================
% Fgraph = mike_results_ui('Clear',F,mode)
...
%=======================================================================
% hMP = mike_results_ui('LaunchMP',M,DIM,hReg,hBmp)
...
set(hBmp,'DeleteFcn','mike_results_ui(''delete'',get(gcbo,''UserData''))')
