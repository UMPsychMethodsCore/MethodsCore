function PPI = mike_peb_ppi_spm5(SPM,varargin)
...
if (length(varargin) ~= 5 && length(varargin) ~= 0)
    error('Wrong number of arguments to mike_peb_ppi_spm5');
end
...
if (length(varargin)>0)
    m_file = varargin{1};
    m_include = varargin{2};
    m_weight = varargin{3};
    m_analysis = varargin{4};
    m_name = varargin{5};
else
    m_file = [];
    m_include = [];
