...
persistent oldmapFile hdr map %Define persistents for last usage of
                              %mapFile (defined at end), hdr, and
                              %map. This should significantly speed
                              %up code execution by avoiding
                              %rereading the map if the function is
                              %called repeatedly using the same mapFile

labeled_voxels = zeros(size(mmlist,1),4);
labeled_voxels(:,1:3) = mmlist;

if isempty(oldmapFile) | ~strcmp(oldmapFile,mapFile)
    hdr=spm_vol(mapFile);
...
oldmapFile=mapFile; %Store the current value of mapFile as
                    %oldmapFile so that future calls can run more
                    %efficiently if mapFile is unchanged
