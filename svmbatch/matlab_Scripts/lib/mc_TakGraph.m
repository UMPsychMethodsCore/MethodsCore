...
if ~isfield(in.SVM_ConnectomeResults.SVMSetup,'matrixtype') % Set default
    in.SVM_ConnectomeResults.SVMSetup.matrixtype = 'upper'
end
...
switch a.svmtype
  case 'paired'
    ...
    switch     in.SVM_ConnectomeResults.SVMSetup.matrixtype
      case 'upper'    
        out = data_delta;
      case 'nodiag'
        data_delta_3d = mc_twinstack(data_delta);
        out = twinwin_mask(data_delta_3d,in);
    end
end
...
function out = consensus_calc(in)
if ~isfield(in.SVM_ConnectomeResults.SVMSetup,'matrixtype') % Set default
    in.SVM_ConnectomeResults.SVMSetup.matrixtype = 'upper'
end
...
switch in.SVM_ConnectomeResults.SVMSetup.matrixtype
  case 'nodiag'
    prune2d = in.SVM_ConnectomeResults.LOOCV_pruning{1};
    prune3d = mc_twinstack(prune2d);
    out = all(any(prune3d,3),1);
  case 'upper'
    out = all(in.SVM_ConnectomeResults.LOOCV_pruning{1});
end

function out = twinwin_mask (threed,in)
% Mask an input (3d) generated by mc_twinstack based on which twins win most of the time
% In cases of a tie, it will prefer the upper. It will return a 2D
% Figure out the twin winners
nrow = size(threed,1);
prune2d = in.SVM_ConnectomeResults.LOOCV_pruning{1};
prune3d = mc_twinstack(prune2d);

twinwin = sum(prune3d,1); % calculate how frequently they won
[twinmax, pref ] = max(twinwin,[],3); % this usage is for looking
                                      % along the third dimension

pref2d = repmat(pref,nrow,1);

pref2d_up = pref2d == 1;
pref2d_dn = pref2d == 2;

out(:,:,1) = threed(:,:,1) .* pref2d_up;
out(:,:,2) = threed(:,:,2) .* pref2d_dn;

%Mask should be exclusive, so get rid of the third dimension
out = sum(out,3);
