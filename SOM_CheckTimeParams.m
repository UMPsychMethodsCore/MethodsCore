...
for iRUN = 1:length(TIME.run)
  ...
  if isfield(TIME.run(iRUN),'TrendFLAG') == 0
    SOM_LOG(sprintf('WARNING : Missing TIME.run(%d).TrendFLAG, setting to 1',iRUN));
    TIME.run(iRUN).TrendFLAG = SOM.defaults.TIME.TrendFLAG;
  end
