...
if (ispc)
    ...
    %% Lapack and LCC options files
    lapack_lib = fullfile(matlabRootDir, 'extern', 'lib', OSDir, 'lcc', 'libmwlapack.lib');
    lcc_bat = fullfile(matlabRootDir, 'bin', OSDir, 'mexopts', 'lccopts.bat');
...
for nF = 1:length(files)
    ...
    if (ispc)
        commandToRun = ['mex "', current_file, '" ', largeArrayDims, ' ', PC_MACRO, ' ', SINGLE_SUPPORT_MACRO, ' ', MWSIZE_MACRO, ' "', lapack_lib, '" -f "', lcc_bat, '"'];
    else
