...
if (~strcmpi(modalityType, 'smri'))
    ...
    inputText(numParameters).answerString = str2mat(icatb_preproc_data);
...
if (~strcmpi(modalityType, 'eeg'))
    ...
    inputText(numParameters).answerString =  str2mat('Default Mask', 'Select Mask');
    inputText(numParameters).uiType = 'popup';
    inputText(numParameters).dataType = 'string';
...
inputText(numParameters).answerString = str2mat(icatb_pca_options);
inputText(numParameters).uiType = 'popup';
inputText(numParameters).dataType = 'string';
...
inputText(numParameters).help = struct('title', 'PCA', 'string', 'There are three options like Standard, Expectation Maximization and SVD.');
...
inputText(numParameters).answerString =  str2mat('2', '3');
inputText(numParameters).uiType = 'popup';
inputText(numParameters).dataType = 'numeric';
...
inputText(numParameters).promptString =  'Number Of PC/IC (Step 3)';
inputText(numParameters).answerString =  '0';
inputText(numParameters).uiType = 'edit';
inputText(numParameters).dataType = 'numeric';
inputText(numParameters).tag = 'numOfPC3';
inputText(numParameters).enable = 'inactive';
inputText(numParameters).visible = isHandleVisible;
inputText(numParameters).value = 1;
inputText(numParameters).flag = 'scalar';
inputText(numParameters).help = struct('title', 'PC3', 'string', 'No. of components desired in the third PCA step.');
...
inputText(numParameters).promptString =  'Which Group ICA Analysis You Want To Use?';
inputText(numParameters).answerString =  str2mat('Regular', 'ICASSO');
inputText(numParameters).uiType = 'popup';
inputText(numParameters).dataType = 'string';
...
inputText(numParameters).value = 1;
inputText(numParameters).flag = 'scalar';
inputText(numParameters).help = struct('title', 'Group ICA Analysis Type', 'string', char('1) Regular - ICA is run only once on the data.', '2) ICASSO toolbox is used to determine the reliability of ICA algorithm. ICA algorithm is run several times to determine the algorithmic reliability or stability. Reliable estimates correspond to tight clusters and un-reliable ones do not point to any cluster.'));
