...
%set(findobj(InputHandle, 'tag', 'algorithm'), 'callback', {@icaAlgorithmCallback, InputHandle});
...
try
    ...
    if (~isfield(sesInfo.userInput, 'preproc_type'))
        try
            ...
            selectedVal = inputText(preProcTag).value;
...
matchedInd = strmatch(lower(allAlgorithms{sel_algorithm}), lower({'Infomax', 'Fast ICA', 'SDD ICA', 'Semi-blind Infomax', 'Constrained ICA (Spatial)', 'FBSS'}), 'exact');
...
if strcmpi(allAlgorithms{sel_algorithm}, 'constrained ica (spatial)')
    ...
    if isempty(spatial_references)
        error(['Spatial reference files need to be selected inorder to use multi-fixed ICA algorithm']);
    end
