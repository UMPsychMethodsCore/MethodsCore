...
if ~strcmpi(modalityType, 'eeg')
    ...
    if (strcmpi(modalityType, 'fmri') && removeConstVoxels)
        ...
        mask_ind = remove_constant_voxels(sesInfo.userInput.files, nonZeroInd(:), XYZ, flagMask);
    end
...
function mask_ind = remove_constant_voxels(files, nonZeroInd, XYZ, flagMask)
...
for i = 1:length(files)
    ...
    dat = read_data(files(i).name, XYZ(:, nonZeroInd == 1));
    tmpM = any(diff(dat, 1));
    if (i == 1)
        chk = tmpM;
    else
