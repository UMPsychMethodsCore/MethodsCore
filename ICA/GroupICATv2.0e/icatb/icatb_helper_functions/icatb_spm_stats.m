...
choiceString = str2mat('One sample t-test', 'Two sample t-test');
...
if ~exist('desVal', 'var')
    ...
    if ((numOfSub > 1) && (numOfSess > 1))
        avgRuns = icatb_promptUI('popup', 'Do You Want To Use Average Of Runs?', str2mat('Yes', 'No'), 'numeric', InputHandle);
        if (avgRuns == 2)
            avgRuns = 0;
        end
    end
...
if (~exist('avgRuns', 'var') || (numOfSess == 1))
    avgRuns = 0;
end
...
if strcmpi(desType, 'one sample t-test')
    ...
    if ~exist('group1Name', 'var')
        [group1Name, group1] = selectGroups(listStr, 'group 1');
    end
    ...
    if ~exist('group2Name', 'var')
        [group2Name, group2] = selectGroups(listStr, 'group 2');
    end
    ...
    for nComp = compNumbers
        ...
        if strcmpi(desType, 'one sample t-test')
            ...
            if ~avgRuns
                ...
                group1Files = strcat(outputDir, filesep, avg_runs_dir, filesep, str2mat(subjectICAFiles{nComp, group}));
            end
            ...
            if ~avgRuns
                ...
                % Group 1 files
                group1Files = strcat(outputDir, filesep, avg_runs_dir, filesep, str2mat(subjectICAFiles{nComp, group1}));
                % Group 2 files
                group2Files = strcat(outputDir, filesep, avg_runs_dir, filesep, str2mat(subjectICAFiles{nComp, group2}));
            end
        ...
        if (writeTal >= 2)
            ...
            tmapFile = fullfile(temp_dir, 'spmT_0001.img');
...
for nDataSet = group1
    ...
    nSess = nDataSet - (nSub - 1)*numOfSess;
    count = count + 1;
    group1Files{count} = strcat(outputDir, filesep, deblank(subjectICAFiles(nSub).ses(nSess).name(compNum, :)));
end
