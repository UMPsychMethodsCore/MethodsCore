function figs = icatb_plot_univariate_results(mancovanInfo)
...
for nF = 1:length(results)
    ...
    if (strcmpi(featureName, 'spatial maps'))
        ...
        img_files = write_composite_sig_effects(mancovanInfo.HInfo(1), covariatesToPlot, results(nF).filesInfo.result_files, mancovanInfo.prefix, outputDir, ...
            mancovanInfo.userInput.p_threshold, mancovanInfo.threshdesc);
        countPlot = 0;
        for nCov = 1:length(covariatesToPlot)
            ...
            S = gather_univariate_effects(mancovanInfo, nF, covariatesToPlot{nCov});
    ...
    elseif (strcmpi(featureName, 'timecourses spectra'))
        ...
        nCols = 2;
        for nCov = 1:length(covariatesToPlot)
            S = gather_univariate_stats(mancovanInfo, nF, covariatesToPlot{nCov});
            countPlot = countPlot + 1;
            sh = subplot(length(covariatesToPlot), nCols, countPlot);
            axesH = sh;
            cmap1 = coldhot;
...
function S = gather_univariate_stats(mancovanInfo, index, term)
...
function S = gather_univariate_effects(mancovanInfo, index, term)
