% icatb_eeg_topoplot() - plot a topographic map of a scalp data field in a 2-D circular view 
...
% Usage:
%        >>  icatb_eeg_topoplot(datavector, EEG.chanlocs);   % plot a map using an EEG chanlocs structure
%        >>  icatb_eeg_topoplot(datavector, 'my_chan.locs'); % read a channel locations file and plot a map
%        >>  icatb_eeg_topoplot('example');                  % give an example of an electrode location file
%        >>  [h grid_or_val plotrad_or_grid, xmesh, ymesh]= ...
%                           icatb_eeg_topoplot(datavector, chan_locs, 'Input1','Value1', ...);
% Required Inputs:
...
%   chan_locs         - name of an EEG electrode position file (>> icatb_eeg_topoplot example).
%                       Else, an EEG.chanlocs structure (>> help icatb_eeg_readlocs or >> icatb_eeg_topoplot example)
% Optional inputs:
...
%                        >> icatb_eeg_topoplot(values,'chanlocs','plotgrid',[11 12 0; 13 14 15]);
...
%                       See >> icatb_eeg_topoplot example. If plotrad > 0.5, chans with arc_length > 0.5 
...
%                       see >> icatb_eeg_topoplot 'example' {default: 'off'}
...
%
% Chan_locs format:
%    See >> icatb_eeg_topoplot 'example'
%
% Examples:
%
%    To plot channel locations only:
%    >> figure; icatb_eeg_topoplot([],EEG.chanlocs,'style','blank','electrodes','labelpoint','chaninfo',EEG.chaninfo);
%    
...
%        - icatb_eeg_topoplots may be rotated. From the commandline >> view([deg 90]) {default: [0 90])
%
...
% fixed recent bug that made all icatb_eeg_topoplot colors positive!!
...
% 'emarkersizemark' (preserving backwards compatability as well). tested with /home/www/eeglab/test_icatb_eeg_topoplot.m
%
...
% function was crashing on regular icatb_eeg_topoplot, debuging
%
% Revision 1.219  2004/11/22 05:02:51  scott
% fixing icatb_eeg_topoplot([],EEG.chanlocs,'emarker','o')
%
% Revision 1.218  2004/11/22 04:58:21  scott
% fixed icatb_eeg_topoplot(32,EEG.chanlocs) and icatb_eeg_topoplot([],EEG.chanlocs,'emarker','o')
...
% make overplot axis limits the same as icatb_eeg_topoplot limits
%
...
% replacing with icatb_eeg_topoplottest - added image output, gridscale arg
%
...
% added test for locs_file string to icatb_eeg_readlocs call
%
...
% icatb_eeg_topoplot Version 2.1
...
%   -now each icatb_eeg_topoplot axes use the caxis command again.
...
% 03-15-02 added icatb_eeg_readlocs and the use of eloc input structure -ad 
% 03-25-02 added 'labelpoint' options and allow Values=[] -ad &sm
% 03-25-02 added details to "Unknown parameter" warning -sm & ad

function [handle,Zi,grid,Xi,Yi] = icatb_eeg_topoplot(Values,loc_file,p1,v1,p2,v2,p3,v3,p4,v4,p5,v5,p6,v6,p7,v7,p8,v8,p9,v9,p10,v10,p11,v11)

%
%%%%%%%%%%%%%%%%%%%%%%%% Set defaults %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%

icatb_eeg_icadefs;                 % read defaults MAXicatb_eeg_topoplotCHANS and DEFAULT_ELOC and BACKCOLOR
if ~exist('BACKCOLOR')  % if icatb_eeg_icadefs.m does not define BACKCOLOR
...
squeezefac = 1.0;
MINPLOTRAD = 0.15;      % can't make a icatb_eeg_topoplot with smaller plotrad (contours fail)
VERBOSE = 'off';
MASKSURF = 'off';
  ...
  if isstr(Values)
    if any(strcmp(lower(Values),{'example','demo'}))
      fprintf(['This is an example of an electrode location file,\n',...
               ...
               '13               180    .181       Pz  \n\n',...
                                                             ...
               'In icatb_eeg_topoplot() coordinates, 0 deg. points to the nose, positive\n',...
...
if nargs < 2
  ...
  if ~exist(loc_file)
      fprintf('default locations file "%s" not found - specify chan_locs in icatb_eeg_topoplot() call.\n',loc_file)
      error(' ')
  end
end
if isempty(loc_file)
...
if strcmp(plotgrid,'on')
   ...
   %      fprintf('icatb_eeg_topoplot() warning: ''plotgrid'' channel matrix has duplicate channels\n');
   % end

   if ~isempty(plotchans)
     if intersect(gchans,abs(plotchans))
        fprintf('icatb_eeg_topoplot() warning: ''plotgrid'' and ''plotchans'' have channels in common\n');
     end
   end
end
...
% 
  if isstr(loc_file)
      [tmpeloc labels Th Rd indices] = icatb_eeg_readlocs( loc_file,'filetype','loc');
  elseif isstruct(loc_file) % a locs struct
      [tmpeloc labels Th Rd indices] = icatb_eeg_readlocs( loc_file );
  ...
  if length(Values) < length(tmpeloc) & strcmpi( STYLE, 'blank') % if Values contains int channel indices to mark
      if isempty(plotchans)
          ...
          elseif strcmpi(VERBOSE, 'on')
              fprintf('icatb_eeg_topoplot(): max chan number (%d) in locs > channels in data (%d).\n',...
...
if isempty(headrad)  % never set -> defaults
  if plotrad >= rmax
     ...
     if strcmpi(VERBOSE, 'on')
       fprintf('icatb_eeg_topoplot(): not plotting cartoon head since plotrad (%5.4g) < 0.5\n',...
                                                                    plotrad);
     end
  end
...
% fprintf('icatb_eeg_topoplot(): plotting %d channels\n',length(pltchans));
 ...
 if ~strcmpi(STYLE,'grid') %  not a rectangular channel grid
  ...
  %
   if strcmpi(noplot, 'on') 
    if strcmpi(VERBOSE,'on')
       fprintf('icatb_eeg_topoplot(): no plot requested.\n')
    end
    return;
   end
  %
